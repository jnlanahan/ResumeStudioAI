...
var getPlatformProperties = () => {
  ...
  if (typeof EdgeRuntime !== "undefined") {
    return {
      ...
      "X-Stainless-Runtime-Version": ((_b = globalThis.process) == null ? void 0 : _b.version) ?? "unknown"
    };
  }
...
async function checkCredentialsFileSafety(path2, onWarn = (m) => console.warn(`anthropic-sdk: ${m}`)) {
  if (typeof process === "undefined" || process.platform === "win32")
    return;
  const { fs } = await import("./node.browser-7NHW5P57.js");
  let resolved = path2;
  let st;
  try {
...
async function writeCredentialsFileAtomic(targetPath, data) {
  const { fs, path: path2 } = await import("./node.browser-7NHW5P57.js");
...
var getCredentialsPath = async (config, profile) => {
  ...
  validateProfileName(profileName);
  const { path: path2 } = await import("./node.browser-7NHW5P57.js");
...
var EnvironmentWorker = (() => {
  class EnvironmentWorker2 {
    ...
    async run(signal) {
      ...
      for await (const work of poller) {
        try {
          ...
          if ((_a2 = poller.signal) == null ? void 0 : _a2.aborted)
            throw e;
    ...
    async handleItem(opts) {
      ...
      const environmentKey = ((opts == null ? void 0 : opts.environmentKey) ?? this.environmentKey ?? readEnv("ANTHROPIC_ENVIRONMENT_KEY")) || void 0;
      const workSecret = (opts == null ? void 0 : opts.workSecret) || readEnv("ANTHROPIC_WORK_SECRET") || null;
      if (!workId) {
  ...
  async function _EnvironmentWorker_handleItem2(work, environmentKey, externalSignal) {
    ...
    const agentToolset = await import("./node.browser-G2NCS24K.js");
    let leaseTtlMs;
    let runner;
    const heartbeatPromise = heartbeatLoop(sessionClient, work, lease, log, this.requestOptions, (ttlMs) => {
      leaseTtlMs = ttlMs;
      runner == null ? void 0 : runner._setSendRetryWindow(ttlMs);
    }).catch((e) => {
...
async function heartbeatLoop(client, work, lease, logger, requestOptions, onLeaseTtl) {
  ...
  const beat = async () => {
    ...
    try {
      const resp = await client.beta.environments.work.heartbeat(work.id, { environment_id: work.environment_id, expected_last_heartbeat: last }, { ...requestOptions, headers: buildHeaders([requestOptions == null ? void 0 : requestOptions.headers]), signal: beatCtrl.signal });
      ...
      if (resp.ttl_seconds > 0) {
        ...
        onLeaseTtl == null ? void 0 : onLeaseTtl(ttlMs);
      }
...
var BetaMessageStream = (() => {
  class BetaMessageStream2 {
    ...
    _connected(response) {
      ...
      __classPrivateFieldSet(this, _BetaMessageStream_workspace_id, response == null ? void 0 : response.headers.get("anthropic-workspace-id"), "f");
...
Please migrate to a newer model. Visit https://docs.anthropic.com/en/docs/resources/model-deprecations for more information.`);
      ...
      let timeout = (options == null ? void 0 : options.timeout) ?? this._client._options.timeout;
...
var MessageStream = (() => {
  class MessageStream2 {
    ...
    _connected(response) {
      ...
      __classPrivateFieldSet(this, _MessageStream_workspace_id, response == null ? void 0 : response.headers.get("anthropic-workspace-id"), "f");
